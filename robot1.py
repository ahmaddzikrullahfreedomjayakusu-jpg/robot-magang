...
class RobotNode(Node):
    ...
    def connect_hp(self):
        ...
        while rclpy.ok() and self.running:
            ...
            for hp_ip in HP_IP_CANDIDATES:

                if not (rclpy.ok() and self.running):
                    break
                ...
                try:
                    ...
                    self.get_logger().warning(
                        f"HP connect gagal {hp_ip}:{PORT}: {e}"
                    )
    ...
    def motor_callback(self, msg):
        ...
        if self.sock is None:
            self.get_logger().warning(
                "MOTOR: HP belum connected, command diabaikan"
            )
            return
