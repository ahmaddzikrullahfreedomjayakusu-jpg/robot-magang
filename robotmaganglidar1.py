...
def yolo_worker_loop():
    ...
    if YOLO is None:
        print("[YOLO] ultralytics tidak tersedia, YOLO dinonaktifkan", flush=True)
        return
...
ENABLE_LEGACY_LOCAL_AUTO = False

sock = None
sock_lock = threading.Lock()
ros_motor_pub = None
ros_save_map_pub = None
ros_auto_follow_pub = None
web_auto_follow = False
web_map_status = "MAP READY"
web_map_status_lock = threading.Lock()
...
class RosPathBridge(Node if Node is not None else object):
    ...
    def __init__(self):
        ...
        global ros_save_map_pub
        global ros_auto_follow_pub
        ...
        ros_save_map_pub = self.create_publisher(
            Bool,
            "/save_map_request",
            10
        )

        ros_auto_follow_pub = self.create_publisher(
            Bool,
            "/auto_follow_enabled",
            10
        )
        ...
        self.create_subscription(
            Int16MultiArray,
            "/green_path_motor_cmd",
            self.green_path_motor_callback,
            10
        )

        self.get_logger().info(
            "ROS BRIDGE READY: /odom + /odom_path + /scan_front + /motor_rpm + /green_path_motor_cmd"
        )
    ...
    def green_path_motor_callback(self, msg):
        if len(msg.data) < 2:
            return
        ...
        if not web_auto_follow:
            return

        sent = send_motor_via_ros(
            kiri,
            kanan
        )
...
def control_send_rpm(rpm_kiri, rpm_kanan, force=False):
    ...
    if ros_motor_pub is not None and Int16MultiArray is not None:
        ...
        try:
            ...
            print(
                "[ROS MOTOR] publish gagal: {}".format(e),
                flush=True
            )
            return False
        _last_send = now
...
def send_motor_via_ros(rpm_kiri, rpm_kanan):
    global _last_send
    global _last_cmd
    global _drive_cmd

    kiri = max(0, min(255, int(round(rpm_kiri))))
    kanan = max(0, min(255, int(round(rpm_kanan))))
    cmd = (kiri, kanan)

    with _drive_cmd_lock:
        _drive_cmd = cmd

    if ros_motor_pub is None or Int16MultiArray is None:
        return False
...
def control_send_rpm_direct_hp(rpm_kiri, rpm_kanan, force=False):
    global _last_send
    global _last_cmd
    global sock
    global _drive_cmd
    ...
    if not force:
        if cmd == _last_cmd and (now - _last_send) < SEND_INTERVAL:
            return False
        if now - _last_send < SEND_INTERVAL:
            return False

    packet = make_packet(kiri, kanan)
    ...
    with sock_lock:
        ...
        try:
            ...
            print(
                "[CTRL] Direct HP send failed: {}".format(e),
                flush=True
            )
...
def toggle_robot():
    ...
    if not ENABLE_LEGACY_LOCAL_AUTO:
        web_auto_follow = False
        ...
        with manual_lock:
            manual_mode = False
            manual_cmd = "STOP"
            manual_last_status = "LOCAL AUTO DISABLED"

        with _drive_cmd_lock:
            _drive_cmd = None

        publish_auto_follow_command(False)
        send_motor_via_ros(
            MOTOR_NEUTRAL,
            MOTOR_NEUTRAL
        )
        ...
        print(
            "[LOCAL] SPACE/legacy auto dimatikan, ROS web/manual saja yang pegang motor",
            flush=True
        )
        return False
...
def set_auto_enabled(enabled):
    ...
    ros_ready = (
        ros_auto_follow_pub is not None and
        Bool is not None
    )

    if enabled and not ros_ready:
        web_auto_follow = False
        with manual_lock:
            manual_mode = False
            manual_cmd = "STOP"
            manual_last_status = "ROS AUTO BELUM READY"
        print("[WEB] ROS AUTO BELUM READY", flush=True)
        return

    web_auto_follow = enabled
    ...
    with manual_lock:
        ...
        manual_cmd = "STOP"
        manual_last_status = (
            "ROS GREEN PATH AUTO ON"
            if enabled
            else "ROS GREEN PATH AUTO OFF"
        )

    with _drive_cmd_lock:
        _drive_cmd = None

    publish_auto_follow_command(enabled)
    ...
    print(
        "[WEB] {}".format(
            "ROS GREEN PATH AUTO START"
            if enabled
            else "ROS GREEN PATH AUTO STOP"
        ),
        flush=True
    )


def publish_auto_follow_command(enabled):

    def worker():
        if ros_auto_follow_pub is None or Bool is None:
            return

        for _ in range(8):
            msg = Bool()
            msg.data = bool(enabled)
            ros_auto_follow_pub.publish(msg)
            time.sleep(0.05)
...
def request_save_map_from_web():
    global web_map_status

    ok = False

    if ros_save_map_pub is not None and Bool is not None:
        def worker():
            for _ in range(8):
                msg = Bool()
                msg.data = True
                ros_save_map_pub.publish(msg)
                time.sleep(0.05)
    ...
    with web_map_status_lock:
        web_map_status = (
            "MAP SAVE REQUESTED"
            if ok
            else "ROS BELUM READY"
        )

    print(
        "[WEB] {}".format(web_map_status),
        flush=True
    )
...
def web_remote_state():
    ...
    with web_map_status_lock:
        map_status = web_map_status

    with camera_lock:
        cam_age = (
            time.monotonic() - latest_camera_ts
            if latest_camera_ts > 0.0
            else 999.0
        )
        cam_ok = latest_camera is not None and cam_age < 2.0
    ...
    return {
        ...
        "camera": "OK {:.1f} FPS".format(camera_fps) if cam_ok else "WAIT HP CAMERA",
...
<body>
  <main>
    ...
    <div class="panel">
      ...
      <div>MAP: <code id="map_status">...</code></div>
      <div>CAMERA: <code id="camera">...</code></div>
      <div>U-TURN: <code id="uturn">...</code></div>
      <div>Bridge: <code id="bridge">...</code></div>
    </div>
    <div class="grid">
      <button class="auto" onclick="cmd('/auto/start')">AUTO JALUR HIJAU</button>
      <button class="stop" onclick="cmd('/auto/stop')">STOP AUTO</button>
    </div>
    <button class="save" onclick="cmd('/map/save')">SIMPAN MAP + JALUR HIJAU</button>
    <div class="grid">
  ...
  <script>
    ...
    async function refresh() {
      ...
      document.getElementById('map_status').textContent = s.map_status;
      document.getElementById('camera').textContent = s.camera;
...
class WebRemoteHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == "/map/save":
            ok = request_save_map_from_web()
            self._send(200 if ok else 503, "ok" if ok else "ros not ready")
            return
...
def main():
    ...
    print("[LOCAL] SPACE = STOP safety, legacy auto dimatikan", flush=True)
    ...
    print("Control: WEB HP = AUTO/MANUAL | SPACE safety STOP")
